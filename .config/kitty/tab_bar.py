...
opts = get_options()
icon_fg = as_rgb(color_as_int(Color(125, 60, 152)))
...
def get_battery_cells() -> list:
    try:
        infos = subprocess.getoutput("pmset -g batt").split()
        percent = int(infos[7][:-2])
        status = infos[8][:-1]
...
#             discharging_icons = ""
#             status = f"{discharging_icons[battery_pct // 10]} {battery_pct}% "
#         else:
#             charging_icons = ""
#             status = f"{charging_icons} {battery_pct}% "
#
#         percent_cell = (bat_text_color, str(battery_pct) + "% ")
#         icon_cell = (bat_text_color, "@")
#         return [percent_cell, icon_cell]
#
#     except Exception:
#         status = " "
